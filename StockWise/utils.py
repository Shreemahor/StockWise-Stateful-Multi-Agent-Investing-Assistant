...
def display_state(
    ...
    try:
        ...
        # Handle portfolio
        portfolio = session.state.get("portfolio", [])
        if portfolio and any(portfolio):
            print("📚 Portfolio:")
            for item in portfolio:
                if isinstance(item, dict):
                    item_id = item.get("id", "Unknown")
                    purchase_date = item.get("purchase_date", "Unknown date")
                    print(f"  - {item_id} (purchased on {purchase_date})")
                elif item:  # Handle string format for backward compatibility
                    print(f"  - {item}")
        else:
        ...
        if interaction_history:
            print(type(interaction_history))
